...
public class EmployeeRepositoryImpl implements EmployeeRepository {
	...
	public Employee loginUser(String username, String hashed) throws SQLException {
		...
		ResultSet resultSet = null;
		try {
			String query = "SELECT EMPLOYEE_ID, EMPLOYEE_ID, EMAIL, FULLNAME FROM EMPLOYEE WHERE EMAIL = ? AND PASSWORD = ?";
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setString(1, username);
			preparedStatement.setString(2, hashed);
			resultSet = preparedStatement.executeQuery();
	...
	public EmployeeUpdate searchEmployee(String employeeIdNumber) throws SQLException {
		...
		try {
			String query = "SELECT EMPLOYEE_ID, FULLNAME, EMAIL, PROJECT_ENGAGEMENT_ID, ROLL_IN_DATE, ROLL_OFF_DATE FROM EMPLOYEE WHERE EMPLOYEE_ID = ?";
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setString(1, employeeIdNumber);
			resultSet = preparedStatement.executeQuery();
	...
	@Override
	public boolean saveEmployeeLeave(List<EmployeeLeave> employeeLeaveList, boolean draft, String empID, String fyID)
			throws SQLException {
		Connection connection = connectionPool.getConnection();
		PreparedStatement preparedStatement = null;
		connection.setAutoCommit(false);
	...
	@Override
	public boolean updateEmployeeLeave(EmployeeLeave emp, boolean draft) throws SQLException {
	...
	public boolean isValidHoliday(String currLeaveStatus, String leaveDate) throws ParseException, OpumException {
		boolean status = false;
		HolidayRepositoryImpl holidayRepo = new HolidayRepositoryImpl();
		Holiday holiday = new Holiday();
		holiday.setDate(leaveDate);
		if (holidayRepo.checkIfHolidayExisting(holiday)) {
