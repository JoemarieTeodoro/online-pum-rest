...
public class EmployeeRepositoryImpl implements EmployeeRepository {
	...
	public Employee loginAdmin(String username, String hashed) throws SQLException {
		...
		try {
			String query = "SELECT EMPLOYEE_ID, EMPLOYEE_ID, EMAIL, ISADMIN, FULLNAME FROM EMPLOYEE WHERE EMAIL = ? AND PASSWORD = ? AND ISADMIN = 1";
			preparedStatement = connection.prepareStatement(query);
	...
	@Override
	public Employee saveEmployeeRole(String employeeSerial) throws BatchUpdateException, SQLException {
		Connection connection = connectionPool.getConnection();
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		logger.info("Employee Serial: " + employeeSerial);
		try {
			connection.setAutoCommit(false);
