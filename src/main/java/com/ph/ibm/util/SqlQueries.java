...
public class SqlQueries {
	...
	public static final String SQL_QUERY_GET_EMPLOYEE_UTILIZATION_HOURS = "SELECT A.DATE, A.WEEKDAY, (SELECT CASE " + 
    		"	WHEN (A.ACTUALHOURS is null && A.IS_HOLIDAY = 0) THEN A.FORECASTHOURS " + 
    		"	WHEN (A.ACTUALHOURS is null && A.IS_HOLIDAY = 1) THEN 'HO' " + 
    		"    ELSE A.ACTUALHOURS END) AS HOURS " + 
    		"FROM ( " + 
    		"SELECT FY.DATE, DAYNAME(FY.DATE) AS WEEKDAY, FY.IS_HOLIDAY, FY.VALUE AS FORECASTHOURS, " + 
    		"	(SELECT (CASE " + 
    		"		WHEN ((LEAVE_TYPE = 'RC'|| LEAVE_TYPE = 'CDO' || LEAVE_TYPE = 'HO' ) && STATUS = 'APPROVED') THEN HOURS " + 
    		"		WHEN ((LEAVE_TYPE = 'SL'|| LEAVE_TYPE = 'VL' || LEAVE_TYPE = 'EL' ) && STATUS = 'APPROVED') THEN LEAVE_TYPE " + 
    		"		ELSE HOURS " + 
    		"		END) FROM EMPLOYEE_LEAVE  " + 
    		"        WHERE EMPLOYEE_ID = ? AND YEAR_ID = ?  " + 
    		"        AND EMPLOYEE_LEAVE.LEAVE_DATE = FY.DATE " + 
    		"	) AS ACTUALHOURS " + 
    		"FROM FY_TEMPLATE FY " + 
    		"WHERE FY.YEAR_ID = ? " + 
    		") AS A " + 
	...
	public static final String SQL_QUERY_GET_MONTH_END_DATE = "SELECT END_DATE FROM MONTH WHERE YEAR_ID = ? AND MONTH_ID = ?";
	
	public static final String SQL_QUERY_GET_EMPLOYEE_LIST_REPORT_DETAILS = "SELECT P.NAME AS PROJECTNAME, E.EMPLOYEE_ID AS SERIALNUMBER, E.FULLNAME AS\r\n" + 
    		"RESOURCENAME, E.ROLL_IN_DATE AS ROLLINDATE, E.ROLL_OFF_DATE AS ROLLOFFDATE\r\n" + 
    		"FROM EMPLOYEE E \r\n" + 
    		"LEFT JOIN PROJECT_ENGAGEMENT PE ON E.PROJECT_ENGAGEMENT_ID = PE.PROJECT_ENGAGEMENT_ID\r\n" + 
    		"LEFT JOIN PROJECT P on PE.PROJECT_ID = P.PROJECT_ID;";
	
	public static final String SQL_QUERY_GET_HOLIDAYS = "SELECT HOLIDAY_ID, NAME, DATE, CREATEDATE, CREATEDBY, UPDATEDATE, UPDATEDBY FROM HOLIDAY "
			+ "WHERE YEAR_ID = ? ORDER BY DATE; ";
	
	public static final String SQL_QUERY_SAVE_MONTHEND = "INSERT INTO month (Month_ID, Month_Name, End_Date, YEAR_ID) VALUES (?,?,?,?) ;";
    
	public static final String SQL_QUERY_GET_MONTHEND_DETAILS = "SELECT MONTH_ID, MONTH_NAME, END_DATE, YEAR_ID FROM MONTH WHERE YEAR_ID = ? ;";

	public static final String SQL_QUERY_UPDATE_MONTH_END_DATE = "UPDATE month SET END_DATE = ? WHERE MONTH_NAME = ? AND YEAR_ID = ? ;";
}
